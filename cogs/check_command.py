    ...
    async def timings(self, ctx: discord.ApplicationContext, timings: str):
        ...
        for word in words:
            if word.startswith("https://timin") and "/d=" in word:
                word.replace(
                    "/d=", "/?id="
                )  # this seems to be a common issue when people post their links
            if word.startswith("https://timin") and "/?id=" in word:
                timings_url = word
                embed_var.url = timings_url
                break
            if word.startswith(
                "https://www.spigotmc.org/go/timings?url="
        ...
        try:
            try:
                onlineMode = request["timingsMaster"]["onlinemode"]
                if onlineMode == False:
                    embed_var.add_field(
                        name="❌ Online-mode",
            ...
            try:
                ...
                if "version" in TIMINGS_CHECK and version:
                    ...
                    if version_result:
                        if (
                            compare_versions(version_result, TIMINGS_CHECK["version"])
                            == -1
                        ):
                ...
                if "-XX:+UseZGC" in flags:
                    ...
                    if "-Xmx" in flags:
                        ...
                        while index < len(request["timingsMaster"]["data"]):
                            timed_ticks = request["timingsMaster"]["data"][index][
                                "minuteReports"
                            ][0]["ticks"]["timedTicks"]
                            player_ticks = request["timingsMaster"]["data"][index][
                                "minuteReports"
                            ][0]["ticks"]["playerTicks"]
                            ...
                            index = index + 1
                        if (
                            1000 * max_online_players / int(max_mem) > 6
                            and int(max_mem) < 10000
                        ):
                            embed_var.add_field(
                                name="❌ Low memory",
            ...
            try:
                ...
                for handler in handlers:
                    ...
                    if handler_name.startswith(
                        "Command Function - "
                    ) and handler_name.endswith(":tick"):
                        handler_name = handler_name.split("Command Function - ")[
                            1
                        ].split(":tick")[0]
                        embed_var.add_field(
                            name=f"❌ {handler_name}",
            ...
            plugins = (
                ...
                if "plugins" in request["timingsMaster"]
                else None
            )
            server_properties = (
                request["timingsMaster"]["config"]["server.properties"]
                if "server.properties" in request["timingsMaster"]["config"]
                else None
            )
            bukkit = (
                request["timingsMaster"]["config"]["bukkit"]
                if "bukkit" in request["timingsMaster"]["config"]
                else None
            )
            spigot = (
                request["timingsMaster"]["config"]["spigot"]
                if "spigot" in request["timingsMaster"]["config"]
                else None
            )
            paper = (
                request["timingsMaster"]["config"]["paper"]
                if "paper" in request["timingsMaster"]["config"]
                else None
            )
            purpur = (
                request["timingsMaster"]["config"]["purpur"]
                if "purpur" in request["timingsMaster"]["config"]
                else None
            )
            if not YAML_ERROR:
                    ...
                    for server_name in TIMINGS_CHECK["plugins"]:
                        if server_name in request["timingsMaster"]["config"]:
                            for plugin in plugins:
                                for plugin_name in TIMINGS_CHECK["plugins"][
                                    server_name
                                ]:
                                    if plugin == plugin_name:
                                        stored_plugin = TIMINGS_CHECK["plugins"][
                                            server_name
                                        ][plugin_name]
            ...
            try:
                for plugin in plugins:
                    ...
                    if (
                        authors is not None
                        and "songoda"
                        in request["timingsMaster"]["plugins"][plugin][
                            "authors"
                        ].casefold()
                    ):
                        if plugin == "EpicHeads":
...
def eval_field(
    ...
    try:
        for option_data in option:
            ...
            for config_name in dict_of_vars:
                if (
                    config_name in option_data["value"]
                    and not dict_of_vars[config_name]
                ):
                    add_to_field = False
                    break
            if add_to_field:
                """f strings don't like newlines so we replace the newlines with placeholder text before we eval"""
                option_data["value"] = eval(
                    'f"""' + option_data["value"].replace("\n", "\\|n\\") + '"""'
                ).replace("\\|n\\", "\n")
                embed_var.add_field(
...
def create_field(option):
    field = {"name": option["name"], "value": option["value"]}
    if "prefix" in option:
